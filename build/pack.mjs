...
// Bundle DuckDB-Wasm from the (immutable) npm package into a single self-contained
// ESM served same-origin from the deployed site, written as duckdb-wasm.bundle.js.
// This removes the runtime dependency on jsDelivr's /+esm transform, whose
// regenerated bytes drift across toolchain updates and repeatedly broke the
// loader's pinned SRI check. npm package versions are immutable, so the bundled
// output is stable and needs no SRI. Requires the esbuild + @duckdb/duckdb-wasm
// devDependencies to be installed (npm ci) in the project root.
async function bundleDuckDbWasm(outDir) {
  let esbuild;
  try {
    esbuild = await import('esbuild');
  } catch (err) {
    throw new Error(
      'esbuild is required to bundle DuckDB-Wasm but is not installed. ' +
      'Run `npm ci` (or `npm install`) in the duckblind project root before packing. ' +
      `(${err.message})`
    );
  }
  const outfile = path.join(outDir, 'duckdb-wasm.bundle.js');
  await esbuild.build({
    stdin: {
      contents: "export * from '@duckdb/duckdb-wasm';",
  ...
  console.log(`  Bundled DuckDB-Wasm (same-origin): ${outfile} (${Math.round(size / 1024)}KB)`);
}
...
INSERT INTO site.vfs (path, mime, content, size) VALUES ('${escapedPath}', '${escapedMime}', from_hex('${hexContent}'), ${content.length});`;
  ...
  if (fs.existsSync(loaderSrc)) {
    ...
    // Bundle DuckDB-Wasm same-origin so the loader's `import('duckdb-wasm.bundle.js')`
    // resolves against our own origin instead of jsDelivr's drift-prone /+esm.
    await bundleDuckDbWasm(outDir);
  } else {
