...
1. Imports DuckDB-Wasm v1.32.0 from a **same-origin** bundle (`duckdb-wasm.bundle.js`) produced at build time; the worker + wasm binaries still load from CDN (SRI-verified)
...
- **DuckDB-Wasm ESM is bundled same-origin, not loaded from a CDN.** `build/pack.mjs` runs esbuild over the immutable `@duckdb/duckdb-wasm` npm package (a devDependency) to emit `duckdb-wasm.bundle.js` into the output, and the loader imports it same-origin — so it needs no SRI. This replaced loading jsDelivr's `/+esm` transform, whose regenerated bytes drifted across their toolchain updates and repeatedly broke the pinned SRI hash. The action runs `npm ci` to install esbuild before packing.
- The worker scripts and WASM binaries are still fetched from jsDelivr and SRI-verified (they are raw, byte-stable package files, unlike the `/+esm` transform). If you bump the DuckDB-Wasm version, bump the `@duckdb/duckdb-wasm` devDependency AND regenerate the worker/wasm hashes in `client/index.html`:
  ```bash
  # MVP worker, EH worker, MVP WASM, EH WASM
  for p in "dist/duckdb-browser-mvp.worker.js" "dist/duckdb-browser-eh.worker.js" "dist/duckdb-mvp.wasm" "dist/duckdb-eh.wasm"; do
